#! /bin/sh
# Delete pending jobs in SLURM queue
...
# awk -v username="$USERNAME" '$4 == username && $5 == "PD" { print $1 }'
#
...
# scancel $job_id
    ...
    USERNAME=$(whoami)
    echo "Deleting all pending jobs owned by $USERNAME"
    for job_id in $(squeue -u $USERNAME | awk -v username="$USERNAME" '$4 == username && $5 == "PD" { print $1 }' | cut -d . -f 1); do
        scancel $job_id
    done
